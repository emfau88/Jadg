...
class Animal {
    ...
    draw() {
        ...
        // hog + winterhog asset faces right by default → flip when moving left
        // other assets face left by default → flip when moving right
        const hogTypes = ['hog', 'winterhog'];
        if (hogTypes.includes(this.type.name)) {
            const dir = this.isKamikaze ? this.baseVx : this.normalVx;
            if (dir < 0) ctx.scale(-1, 1);
        } else {
            // duck, turkey, deer, rehbock: asset faces left → flip when going right
...
function drawHunter() {
    // Wähle Winter-Jäger ab Level 5, fallback auf normalen Jäger
    const hunterKey = currentLevel >= 5 ? 'hunterWinter' : 'hunter';
    const hunter = images[hunterKey] || images.hunter;
...
function nextLevel() {
    ...
    if (currentLevel === 5) {
        setTimeout(() => showWarningText('❄️ WINTEREINBRUCH!'), 600);
    }
